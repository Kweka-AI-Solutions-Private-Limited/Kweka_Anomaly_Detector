"""
evaluate_patchcore_layer1.py

Controlled evaluation script for PatchCore Layer1 2x Spatial Reduction Experiment.
Guarantees 256x256 test image preprocessing BEFORE entering backbone.
Applies project-level 2x spatial feature pooling (stride=2 AvgPool2d).
...
RUN_INFO_FILE = PROJECT_ROOT / "outputs" / "patchcore_baseline" / "baseline_run_screw_wideresnet50_layer1_spatial2x_005.json"
OUTPUT_DIR    = PROJECT_ROOT / "outputs" / "patchcore_baseline" / "screw_wideresnet50_layer1_spatial2x_005_localization"
...
print("=" * 70)
print("PATCHCORE LAYER1 CONTROLLED EVALUATION (2X SPATIAL REDUCTION)")
print("=" * 70)
print(f"Checkpoint: {CHECKPOINT}")
if not CHECKPOINT.exists():
    raise FileNotFoundError(f"Run run_patchcore_layer1.py first.\nMissing: {CHECKPOINT}")
...
# Apply exact same project-level 2x spatial reduction: AvgPool2d(kernel_size=3, stride=2, padding=1)
...
print("\n" + "=" * 70)
print("PATCHCORE LAYER1 (2X SPATIAL REDUCTION) FINAL RESULTS")
print("=" * 70)
...
print("\n" + "=" * 70)
print("PATCHCORE LAYER1 EVALUATION SCRIPT COMPLETE")
print("=" * 70)
