"""
run_patchcore_layer1.py

Controlled PatchCore Layer1 2x Spatial Reduction Experiment.
Config:
  ...
  Spatial Reduction:  2x via AvgPool2d(kernel_size=3, stride=2, padding=1)
                       Reduces raw layer1 (64x64) -> (32x32) = 1,024 patches/image
...
CHECKPOINT_PATH = OUTPUT_DIR / "patchcore_screw_wideresnet50_layer1_spatial2x_005.ckpt"
RESULTS_PATH     = OUTPUT_DIR / "baseline_run_screw_wideresnet50_layer1_spatial2x_005.json"
...
print("=" * 70)
print("PATCHCORE LAYER1 2X SPATIAL REDUCTION EXPERIMENT (320 IMAGES, 5% CORESET)")
print("=" * 70)
...
print(f"Spatial Reduction: 2x (64x64 -> 32x32 via stride=2 AvgPool2d)")
...
patches_per_image = 32 * 32  # 2x spatial reduction produces 32x32 = 1,024 patches
...
print(f"  Raw Spatial Map          : 64 x 64 (layer1 @ 256x256 input)")
print(f"  Reduced Spatial Map (2x) : 32 x 32 = {patches_per_image} patches/image")
...
# ── PatchCore Model Setup & Project-Level 2x Spatial Reduction ─
...
# Project-Level 2x Spatial Reduction: AvgPool2d(kernel_size=3, stride=2, padding=1)
# Reduces raw layer1 feature maps from 64x64 to 32x32 (1024 patches/img) before coreset stage
...
# ── Memory Bank Fitting ───────────────────────────────────────
print("\n[4/4] Building PatchCore Layer1 2x memory bank (320 images, 5% coreset)...", flush=True)
print("Extracting 32x32 layer1 feature representations from pre-resized 256x256 GOOD training images...")

start_time = time.time()
hb_stop = start_heartbeat("PatchCore layer1 2x spatial memory bank construction")
try:
...
print("\n" + "=" * 70)
print("PATCHCORE LAYER1 2X SPATIAL TRAINING COMPLETE — Ready for evaluate_patchcore_layer1.py")
print("=" * 70)
