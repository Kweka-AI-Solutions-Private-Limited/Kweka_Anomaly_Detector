...
Controlled evaluation script for DRÆM Baseline Experiment.
Loads trained checkpoint: outputs/draem_baseline/draem_screw_256_001.ckpt
...
CHECKPOINT    = PROJECT_ROOT / "outputs" / "draem_baseline" / "draem_screw_256_001.ckpt"
RUN_INFO_FILE = PROJECT_ROOT / "outputs" / "draem_baseline" / "baseline_run_screw_draem_001.json"
OUTPUT_DIR    = PROJECT_ROOT / "outputs" / "draem_baseline" / "screw_draem_001_localization"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 70)
print("DRAEM BASELINE CONTROLLED EVALUATION")
print("=" * 70)
print(f"Checkpoint: {CHECKPOINT}")
if not CHECKPOINT.exists():
    raise FileNotFoundError(f"Run train_draem.py first.\nMissing: {CHECKPOINT}")
...
if RUN_INFO_FILE.exists():
    with open(RUN_INFO_FILE, "r") as f:
        ...
        train_time = run_meta.get("train_time_seconds", 0.0)
        inference_time = run_meta.get("inference_time_seconds", 0.0)
        num_train_good = run_meta.get("number_of_training_good_images", 320)

# ── 1. Inference ──────────────────────────────────────────────
print("\n[1/5] Running test inference with DRÆM model ...")
datamodule = MVTecAD(root=str(DATASET_ROOT), category=CATEGORY, eval_batch_size=1, num_workers=0)
datamodule.setup()
...
print("\n" + "=" * 70)
print("DRAEM BASELINE FINAL RESULTS")
print("=" * 70)
...
print("\n" + "=" * 70)
print("DRAEM BASELINE EVALUATION SCRIPT COMPLETE")
print("=" * 70)
